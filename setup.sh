...
then
echo ""
echo -e $red "It was not possible to found searchsploit executable in : $sspd"
echo ""
...
echo "bash $sspd" | tee -a $config $log > /dev/null 2>&1
chk="$path/logs/check"
if [ -f "$file" ]
then
ct=`sed -n 1p $chk`
...
ln -s $msfv/msfvenom /usr/local/sbin/msfvenom > /dev/null 2>&1
echo "msfconsole" | tee -a $config $log > /dev/null 2>&1
echo "msfvenom" | tee -a $config $log > /dev/null 2>&1
bkf
;;
...
fail=$path/logs/failsetup.log
cfg=$path/config/config.path
